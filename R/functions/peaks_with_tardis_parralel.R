  ...
  function(file_path,
    ...
    if (batch_mode == TRUE) {
      for (batchnr in 1:length(batch_positions)) {
        ...
        #start <- Sys.time()
        data_QC <-
          readMsExperiment(spectraFiles = QC_files, backend = MsBackendMzR())
        ...
        process_component <- function(j) {
          sample_name <- unlist(sample_names)
          ...
          sample_spectra <- filterDataOrigin(spectra, unique(dataOrigin(spectra)))
          sample_spectra <- filterRt(sample_spectra, rtRanges[j, ])
          sample_spectra <- filterMzRange(sample_spectra, mzRanges[j, ])
          
          sfs_agg <- addProcessing(sample_spectra, .sum_intensities)
          ...
          if (length(unique(y)) > 1) {
            ...
            compound_info <- info_compounds[j, ]
        ...
        results_QCs_batch <- do.call(rbind, lapply(1:dim(rtRanges)[1], process_component))
        
        # Create and save the plot for the current component
        if (plots_QC == TRUE) {
          plotQCs(compound_info, output_directory, rt_list, int_list, x_list, y_list)
        }
        ...
        data_samples <-
          readMsExperiment(spectraFiles = sample_files, backend = MsBackendMzR())
